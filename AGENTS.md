...
See `metaculus_bot/llm_configs.py` for the authoritative list (rotates frequently). As of this writing: gpt-5.6-sol, gpt-5.5, claude-opus-4.7, claude-opus-4.8, gemini-3.1-pro-preview, grok-4.5. Do NOT hardcode model names outside `llm_configs.py`. Provider: OpenRouter with automatic key fallback. Roster change 2026-07-15: Fable-5 joined the forecaster roster (was stacker-only; stacking disabled in prod made it idle) and opus-4.6 retired, keeping n=6 and a 2 Anthropic / 2 OpenAI / 1 Google / 1 xAI provider balance. Roster change 2026-07-20: Fable-5 PULLED from the forecaster roster (and the stacker — see below) after it returned message.content=None on 4/4 attempts for Q14333's numeric forecast + a truncated no-JSON-block output on Q578 in the 2026-07-19 test_bot run (suspected fable-5 content classifiers refusing certain question content — fast deterministic empty completions, not timeouts); opus-4.7 takes the slot at xhigh, mirroring the opus-4.8 config, preserving n=6 and the 2 Anthropic / 2 OpenAI / 1 Google / 1 xAI balance. Reconsidering fable-5 is a tracked high-prio follow-up (FUTURE.md).

Support models (also in `llm_configs.py`):

- **Stacker**: `claude-opus-4.8` (primary, since 2026-07-20 when fable-5 was pulled from both roles — see the 2026-07-20 roster-change note above; effort-based adaptive thinking `reasoning={"effort": "xhigh"}`, not a max_tokens budget; the stacker chain remains configured but prod-disabled. Was `claude-fable-5` from 2026-07-07 to 2026-07-20), `gpt-5.6-sol` (cross-provider fallback for independence if Anthropic thrashes; stays at effort=high — it fires on a tight 300s budget). Both `allowed_tries=1` — on stall, we fall back rather than burn budget retrying the same provider. OpenRouter's full effort enum (live-verified 2026-07-15): none/minimal/low/medium/high/xhigh/max; the xhigh-capable slots (gpt-5.6-sol, gpt-5.5, opus-4.7, opus-4.8 forecasters + the opus-4.8 stacker) run at xhigh. "max" is Anthropic-only (one tier above xhigh; OpenAI's ceiling is xhigh and rejects max upstream) — held back on the Anthropic slots for latency.
