...
- **`GOOGLE_API_KEY` — defaults to a Metaculus-billed Google AI Studio key; a personal key exists as backup.** In CI it's stored as `secrets.GEMINI_API_KEY` and surfaced as `GOOGLE_API_KEY` in the workflow env so the `google-genai` SDK picks it up. The key currently in CI bills Metaculus, not the operator (empirical: personal-key debit emails stopped when it was switched in; an earlier note here claiming "no donated Google key exists" was wrong — corrected 2026-07-16). The grounded-search side (`research/gemini_search.py`, `research/targeted.py`) uses whatever key this env var holds. Don't confuse the OpenRouter Gemini path (donated route via `OAI_ANTH_OPENROUTER_KEY`, with the gemini-3.1-pro-preview blocklist exception) with this google-genai path — they're separate keys and separate billing.
