...
**The known-pipeline-bug cohort has one canonical home: `KNOWN_BUG_QIDS` in `performance_analysis/width_monitor.py`.** These are questions whose published forecast came out of a since-fixed pipeline defect rather than judgment, so pooling them into a calibration or miss-ranking row measures the retired bug. It currently holds 43746/43747 (the pre-2026-07-07 open-bound arithmetic bug) and, since 2026-08-25, 43913 (the pre-`9f1175c` discrete max-step cap: all six forecasters stated 79.5-83% on the outcome that resolved, the published CDF carried 20.00% with its first bin pinned at exactly 0.200000 on an 11-point grid — receipts in `scratch/residual_2026-08-24/dossiers/43913_dossier.md`). Import the constant instead of re-hardcoding the ids; every private copy in a round's analysis scripts has drifted from it at least once. Nothing excludes the cohort by default — a caller passes it explicitly (`--exclude-qids known_bug`) and the excluded count is rendered per row, so an exclusion is a visible choice rather than a silent filter.
...
**Degraded-run cohort (dry-donated-key incident, 2026-07-26 → 07-28) — standing scoring exclusions.** The pre-fix dry-key window published eleven triple-era questions on a thinned ensemble; exclude them from headline aggregates and report them separately. Full 1-of-3 publishes (gemini only, the personal-key-pinned slot): question ids **44870–44877** (post ids 44721–44728 — the two id spaces differ and collide numerically with other questions, so join via `performance_analysis/id_mapping`, never raw integers). Partial 2-of-3: **44841, 44856, 44912**. On the research side 44841/44856 are degraded identically to the full cohort (native search errored, both gap-fill passes dead), so a research-conditioned cut must exclude them alongside it even though the forecaster-count tagging separates them. The first two resolved 2026-08, both favorably — 44870 peer +14.38 (published on gemini alone), 44841 +21.54 — a two-question favorable draw, not evidence that degraded publishes are fine. Receipts: `scratch/residual_2026-08-24/degraded_cohort.json`.
