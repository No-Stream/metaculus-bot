...
A forecasting bot for Metaculus. It runs an ensemble of frontier LLMs, gathers evidence from several research sources in parallel, and turns the models' answers into a single prediction it publishes to Metaculus, or to Mantic's Crucible competition in `--mode mantic`.
...
- **Numeric and date CDF pipeline.** On a Metaculus question, and on a Mantic grid too wide for per-bin elicitation, each forecaster declares the canonical percentile set (`STANDARD_PERCENTILES` in `metaculus_bot/numeric/config.py`) and the bot turns them into a PCHIP CDF on the `PCHIP_CDF_POINTS` grid under the server's constraints (min/max step per bin, bound pinning, strictly increasing). A Mantic question with 31 bins or fewer is elicited per bin instead (`elicit_per_bin`: one probability per bin, built into a CDF by `numeric/pmf_cdf.py`) and its members are pooled by the pointwise mean. Date questions run the same pipeline on an epoch-seconds axis (`numeric/date_axis.py`), and every published Mantic numeric or date CDF is lifted to at least 5% beyond each open bound, as far as the other tail leaves room (`numeric/out_of_range_floor.py`), because Mantic scores out-of-range resolutions against a fixed 5% reference.
...
- **`mantic.py`**: the Mantic (Crucible) platform client, a `MetaculusClient` subclass pointed at competitions.mantic.com that `--mode mantic` swaps in.
- **`question_platform.py`**: which platform a question is on, read off its `page_url`; the prompts, the per-bin gate and the tail floor key on it.
- **`time_budget.py`**: the per-question time budget derived from the question's own close time (intake skip, fast path, research-phase deadline).
...
- **`numeric/`**: `pipeline.py`, `pchip_cdf.py`, `tail_widening.py` (the percentile → PCHIP CDF pipeline), `date_axis.py` (date questions on the epoch-seconds axis), `pmf_grid.py` + `pmf_cdf.py` (per-bin elicitation on the small Mantic grids), `out_of_range_floor.py` (the Mantic 5% floor beyond each open bound).
- **`comment/`**: `formatting.py`, `markers.py`, `trimming.py`, which assemble the comment published with each forecast.
...
- `GAP_FILL_ENABLED`: v1 gap-fill, where an analyzer lists factual gaps and parallel native searches resolve them.
...
│   ├── mantic.py                   # Mantic (Crucible) platform client for --mode mantic
│   ├── question_platform.py        # Metaculus or Mantic, read off page_url
│   ├── time_budget.py              # Close-derived per-question time budget
...
│   │                               #   date_axis, pmf_grid + pmf_cdf (per-bin Mantic grids),
│   │                               #   out_of_range_floor (Mantic 5% open-tail floor)
...
- Question types: `BinaryQuestion`, `NumericQuestion`, `MultipleChoiceQuestion`, `DateQuestion` (forecast on its epoch-seconds view, `numeric/date_axis.py`)
