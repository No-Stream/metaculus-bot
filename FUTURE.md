...
- **C1, per-bin PMF elicitation for enumerable grids: SHIPPED 2026-09-09.** On a Mantic numeric
  ...
  (`unrecorded` marks a bug), and `EXTRACTION_RUNG` logs `qtype=pmf` for the ladder that parsed
  the block (the question type is joined from the member line). Left as is, deliberately: the published comment (the
...
- **Rejected alternative to C1: an `excluded_bins` list on the percentile block** (raised in
  ...
  after the PCHIP build and re-run the floor blend: one schema field and one post-processing
  step, no new template or ladder, and it keeps `sanitize_percentiles`, the unit-mismatch guard
  and the smooth-shape prior. Rejected because eligibility lives only in the criteria prose, so
  neither the prompt builder nor the gate can know which bins to exclude or check the list a
  model returns; because it cannot express a genuinely multimodal per-bin view or a count grid too
  coarse for thirteen distinct percentiles; and because under the per-bin ask the forecaster is
  simply told to give an excluded bin 0 (`_PER_BIN_SCORING_RULE`), which solves the motivating
  case with no extra field.
...
- **A MEMBER_FORECAST line for a per-bin member whose CDF build raises: reviewed 2026-09-09,
  declined on the merits (Wave C review finding F28).** The percentile path emits the marker
  before its fail-shut raise; on the per-bin path the guards run inside `build_pmf_distribution`,
  before the marker. Every `_validated_declaration` raise there is unreachable from the runner
  (`extract_pmf` already enforces the same rules) and `validate_grid_cdf` is a fail-shut check no
  input should reach; the drop is in `FORECASTER_DROPS` as `error_other` and the declaration is
  in the LLM OUTPUT dump, so splitting the build, or a try/except-log-reraise for a bug-only
  path, leaves the code worse.
- **The supply probe's preflight as a `verify_identity` partial on `PlatformProbe`: reviewed
  2026-09-09, declined on the merits (Wave C review finding F30).** A `partial` in a frozen
  dataclass binds the function at import time and breaks the six existing monkeypatches (the
  repo's patch-where-used trap); finding F20 fixed the same seam with a `base_url` field instead,
  vetted by `verify_api_identity` at call time, so the token still goes to exactly the vetted
  host.
- **Letting Mantic's public snapshot override an empty `my_forecasts` block: reviewed 2026-09-09,
  declined on the merits (Wave C review finding F31).** The token's block wins whenever it
  answers. Upstream's `request_user_forecasts` prefetch has no post-status gate and no `end_time`
  filter, so an empty block on a closed or resolved post means the account genuinely never
...
5. **The effect of the two Phase 1 prompt rules the next-season bundle adds**, `_SOFT_CLOCK_RULE`
   ...
   history-repeats comparison below has nothing to measure. `docs/prompts.md` carries the revert recipe, in the `_HISTORY_DISCHARGED_RULE` bullet. Nothing about either can be measured offline, so the first read is the residual
...
- **Not cleanly over- or under-confident: it flips by era.** Bayesian logistic slope:
  ...
  slope (1.17), which is why past pooled reads were contradictory. Even within spring the
  early/late split flips 0.64 → 1.25 (roster churn inside one tournament).
...
A sixth, accepted deliberately on 2026-09-02 and **FIXED 2026-09-08 on the Mantic branch (Phase 2
review finding F24, commit `8fc7b06`)**: the ft-fallback numeric builder's CLOSED-bound path used to
skip `safe_cdf_bounds` entirely. In `create_fallback_numeric_distribution`
(`metaculus_bot/numeric/pchip_processing.py`) the `BoundSafeNumericDistribution.get_cdf` override
returned upstream's CDF unchanged when neither bound was open, so no min-step, max-step or endpoint
enforcement ran on that output and no `CDF_MAXSTEP_CLIP` marker could fire; the 2026-08-31 forge
reviewer measured a 0.2034 bin against the platform's 0.2 cap on that path
(`scratch/residual_2026-08-31/forge_report_resids-sept1.md` R1). It was accepted then as
compound-rare (closed bounds AND a PCHIP failure AND stacking enabled, with stacking prod-disabled
and zero measured fires in the archive; operator decision (b) in
`scratch/residual_2026-08-31/SYNTHESIS.md`). The Mantic readiness review reopened it: the
lone-survivor path publishes the fallback directly with no stacking involved, and a tight
declaration (normal, sd 0.6 on a [0, 100] grid) on the modal Mantic date shape produced a 0.297 bin
against the 0.2 cap and an HTTP 400. The override now runs `safe_cdf_bounds` with the grid's own
step limits whatever the bound shape; it pins nothing on a closed bound and `enforce_min_steps`
caps at [0.0, 1.0], so the closed endpoints survive exactly. `safe_cdf_bounds` is again the single
choke point `docs/numeric_pipeline.md` describes.
...
- **Files over the monolith threshold, re-measured 2026-08-25** (the 2026-07-18 numbers were
  stale in both directions). Closed: `research/timeseries_anchor.py` split in `e4bddae` (1,293 →
...
- **Promote the shared SSRF/fetch primitives into `http_fetch.py`** (still open, and the coupling
  has widened). `agentic/tools.py` reaches into `resolution_source.py`'s private functions at six call
...
and now pay on just the opus-4.8 forecaster after the evening sol drop) is unmeasured. The audit's
...
### financial_data / ts-anchor: calendar time converted on a hardcoded trading-day density (FIXED 2026-08-24 in `e6ae276`; class swept 2026-08-25 in `c577231`)
