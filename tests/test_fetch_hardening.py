...
# ---------------------------------------------------------------------------
# Socket-timeout injection
# ---------------------------------------------------------------------------


def test_inject_get_timeout_adds_default_when_caller_omits():
    """Bare `requests.get(url, params=...)` should pick up timeout=X."""
    from forecasting_tools.helpers import metaculus_api as ft_api

    captured = {}
    original = ft_api.requests.get

    def fake_get(*args, **kwargs):
        captured.update(kwargs)
        return MagicMock()

    ft_api.requests.get = fake_get
    try:
        with fetch_hardening._inject_get_timeout(7.5):
            ft_api.requests.get("http://example.com", params={"x": 1})
    finally:
        ft_api.requests.get = original

    assert captured.get("timeout") == 7.5


def test_inject_get_timeout_respects_caller_supplied_timeout():
...
def test_inject_get_timeout_restores_original_on_exit():
    from forecasting_tools.helpers import metaculus_api as ft_api

    original = ft_api.requests.get
    with fetch_hardening._inject_get_timeout(1.0):
        assert ft_api.requests.get is not original
    assert ft_api.requests.get is original


def test_inject_get_timeout_restores_original_on_exception():
    from forecasting_tools.helpers import metaculus_api as ft_api
...
def _reset_fetch_hardening_state(monkeypatch):
    """Snapshot + restore patched methods and sentinel via monkeypatch."""
