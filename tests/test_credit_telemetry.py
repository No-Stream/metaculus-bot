...
The per-role dollar ledger behind ``CREDIT_ROLE_SPEND`` is pinned in
tests/test_credit_role_spend.py.
"""
...
class TestFloorCheck:
    ...
    def test_nan_remaining_is_unreported_not_above_floor(self, monkeypatch, caplog) -> None:
        ...
        # A balance missing at one end routes the delta through the usage fallback; the spend figure survives.
...
class TestUnsettledSpendDisclosure:
    ...
    def test_zero_delta_on_the_usage_branch_is_flagged_unsettled(self, monkeypatch, caplog) -> None:
        """The exact production shape: usage identical at both ends (the settlement window had
        not closed), which is the case that reads as free-but-wasn't."""
        _set_keys(monkeypatch, donated=None)
