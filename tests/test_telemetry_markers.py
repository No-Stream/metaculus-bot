...
class TestWindowDeclared:
    def test_fields(self):
        rec = _parse_one(WINDOW_DECLARED_FULL_WINDOW_LINE)
        assert rec["marker"] == "window_declared"
        assert rec["model"] == "openai/gpt-5.6-sol"
        # Both counts harvest as numbers so the ratio cut is arithmetic, not string work.
        assert rec["declared_days"] == 90
        assert rec["actual_days"] == 74.3

    def test_question_ref_is_stamped_in_the_question_id_space(self):
        # forecaster_runners.py emits question.id_of_question, the same space as
        # forecasters_survived, so the join to the survivor set needs no translation.
        rec = _parse_one(WINDOW_DECLARED_FULL_WINDOW_LINE)
        assert rec["qid"] == 43837
        assert rec["qid_kind"] == "question_id"

    def test_full_window_pricing_is_distinguishable_from_remaining_window_pricing(self):
        # The qid 43837 shape versus the correct one, on the same question in the same
        # run: only the declared/actual ratio separates them.
        full = _parse_one(WINDOW_DECLARED_FULL_WINDOW_LINE)
        remaining = _parse_one(WINDOW_DECLARED_REMAINING_ONLY_LINE)
    ...
    def test_one_record_per_member_on_the_same_question(self):
        ...
        assert [r["declared_days"] for r in harvested["window_declared"]] == [90, 74]
