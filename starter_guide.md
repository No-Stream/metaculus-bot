...
> **Deprecated path.** `community_benchmark.py` scores against the community prediction via expected baseline score, which broke when Metaculus removed `aggregations` from the list API — so its scoring is unreliable for newly-fetched questions. Prefer **`backtest.py`** (`make backtest_smoke_test` / `backtest_small` / `backtest_medium` / `backtest_large`), which scores against actual question resolutions. `community_benchmark.py` is retained only so `make benchmark_display` can view old runs.
