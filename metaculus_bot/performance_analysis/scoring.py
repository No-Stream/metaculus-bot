"""Standalone scoring functions for performance analysis.

Inlined from metaculus_bot/backtest/scoring.py to keep this module dependency-free
(no forecasting_tools imports). All functions are pure with no side effects.
"""
...
def binary_log_score(predicted_prob: float, outcome: bool) -> float:
    """Metaculus-style log score for binary questions. Higher is better."""
    p = _clamp_prob(predicted_prob)
    y = 1.0 if outcome else 0.0
...
def _resolution_to_bucket_index(
    ...
    """Map a numeric resolution to a PMF bucket index in [0, n_inbound+1]."""
...
def numeric_log_score(
    ...
    """Metaculus-style PMF-bucket log score for numeric questions. Higher is better."""
...
def mc_log_score(predicted_probs: list[float], correct_option_index: int) -> float:
    """Metaculus-style log score for multiple-choice questions. Higher is better."""
