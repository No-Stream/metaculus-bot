...
registry meet, and its failure logging is pinned to the orchestrator's logger. It
arrives here as the ``run_one`` argument, which is the whole of this module's
coupling to the orchestrator — nothing here reads orchestrator state.
"""
...
async def await_providers_within_deadline(
