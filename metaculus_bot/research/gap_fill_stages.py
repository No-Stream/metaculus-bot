...
``ResearchOrchestrator`` mixes in ``GapFillStages``. The counters these stages bump
(``gap_fill_v2_error_count``, ``research_budget_cut_count``) stay on the orchestrator,
which is what the forecaster and the end-of-run degradation line read; the two
gap-fill modules stay behind function-level imports inside the failure guards, so an
import error in either one degrades the question instead of killing the run.
"""
...
class GapFillStages:
    """Mixin: the two optional gap-fill passes and their budget/error accounting.

    The three members below are DECLARATIONS for the type checker, not defaults:
    ``ResearchOrchestrator`` owns the benchmarking flag, the v2 error counter and the
    budget-cut bookkeeping, and this mixin is never instantiated on its own.
    """

    _is_benchmarking: bool
    gap_fill_v2_error_count: int

    def _record_research_budget_cut(self, question: MetaculusQuestion, *, fast_path: bool) -> None:
        """Declared here for the type checker; ``ResearchOrchestrator`` implements it."""
