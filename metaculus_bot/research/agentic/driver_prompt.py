...
through the dry run, note:
  ...
  - Any question that resolves off a live data source (a tracker, index,
    average, counter, or dashboard): its CURRENT reading and the date it was
    last updated is a verify target, retrieved from the instrument itself.
    Never make a target of what that source will read on the resolution date —
    no source can answer that yet, and the budget is spent for nothing.
