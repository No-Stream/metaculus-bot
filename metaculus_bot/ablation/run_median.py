"""Median-aggregation runner: deterministic baseline for the probabilistic-tools ablation.
...
* Binary: median of probabilities, then ``[BINARY_PROB_MIN, BINARY_PROB_MAX]`` clamp.
* Multiple choice: option-wise median across forecasters per option (preserving the
...
* Numeric: ``aggregate_numeric(predictions, question, method="median")`` —
  the same pointwise-CDF aggregator the production pipeline uses, in median mode.
...
__all__ = ["run_median_for_qid"]

# Sentinel literal stored in ``stacker_model_used`` for ARM_MEDIAN payloads. The
...
def _aggregate_binary(deserialized: list[float]) -> float:
    ...
    median_prob = statistics.median(deserialized)
...
def _aggregate_mc(
    ...
    # PredictedOptionList enforces a sum-to-1 invariant in its validator (within
    # a tolerance), so renormalize BEFORE constructing — option-wise medians are
    # not guaranteed to sum to 1. Clamp first to keep [MC_PROB_MIN, MC_PROB_MAX]
    # in force; ``clamp_and_renormalize_mc`` does the same thing on the resulting
    # list as a belt-and-suspenders pass.
    raw_probs = {name: float(statistics.median(per_option_values[name])) for name in option_order}
...
async def run_median_for_qid(
    *,
    ...
    """Run the ARM_MEDIAN simple-aggregation baseline for one question. Cached per ``(qid, ARM_MEDIAN)``.
    ...
    if len(surviving) < ABLATION_MIN_FORECASTERS:
        # Schema mirrors run_stacker_for_arm's insufficient-forecasters branch
        # so cli/spend code can subscript the same keys regardless of arm.
        error_payload = {
            "success": False,
            "arm": ARM_MEDIAN,
        ...
        cache.write_stacker_output(qid=qid, arm=ARM_MEDIAN, payload=error_payload)
        await asyncio.sleep(0)
    ...
    if isinstance(question, BinaryQuestion):
        ...
        aggregated = await aggregate_numeric(deserialized, question, method="median")
    else:
        raise ValueError(f"Unsupported question type for ARM_MEDIAN: {type(question).__name__}")
    ...
    cache.write_stacker_output(qid=qid, arm=ARM_MEDIAN, payload=success_payload)
